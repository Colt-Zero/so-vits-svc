...
from svcmodules.commons import sequence_mask
...
def save_checkpoint_deepspeed(
    model_engine, optimizer, learning_rate, iteration, checkpoint_path):
    logger.info("Saving model and optimizer state at iteration {} to {}".format(
    iteration, checkpoint_path))
...
def load_filepaths_and_text(filename, split="|"):
  with open(filename, encoding=
        'utf-8' if os.name == 'posix' else 'cp1252') as f: 
...
def get_hparams(init=True):
  ...
  hparams.reset = args.reset
  return hparams

def get_hparams_deepspeed(init=True):
  import deepspeed
  parser = argparse.ArgumentParser()
  parser.add_argument('-c', '--config', type=str, default="./configs/base.json",
                      help='JSON file for configuration')
